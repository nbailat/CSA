...
public class ScantArray {
    ...
    public int getValueAt(int row, int col){
        for (ScantArrayEntry scantArrayEntry : entries){
            if(scantArrayEntry.getRow()==row && scantArrayEntry.getColumn()==col)
            return scantArrayEntry.getValue();
        }
    ...
    public void removeColumn(int col){
        for(int i = entries.size() - 1; i >= 0; i--){
            ...
            entries.remove(i);
            if(scantArrayEntry.getColumn() > col) entries.add(new ScantArrayEntry(scantArrayEntry.getRow(), scantArrayEntry.getColumn() - 1, scantArrayEntry.getValue()));
            if(scantArrayEntry.getColumn() < col) entries.add(new ScantArrayEntry(scantArrayEntry.getRow(), scantArrayEntry.getColumn(), scantArrayEntry.getValue()));
        }
        numColumns--;
